...
for doc, dn in [(MS+TABS+APP, "manuscript+tables"), (MEMO, "memo")]:
    must(doc, dn, n(F["eligible_90_enrolled_days"]), n(F["eligible_analysis"]), n(F["excluded_training_overlap"]), n(F["excluded_not_tier1"]), n(D["eligible_patients"]), n(F["eligible_test"]), n(F["eligible_test_patients"]), pc(F["primary_rate_all"]), pc(F["primary_rate_by_state"]["OHIO"]), pc(F["primary_rate_by_state"]["VIRGINIA"]), pc(F["primary_rate_by_state"]["WASHINGTON"]),
...
must(MS+TABS+APP, "manuscript+tables", n(F["decision_points"]), n(F["patients"]), f"{F['eligible_pct']}%", n(F["eligible_train"]), n(D["train_patients"]), n(F["primary_events_test"]), pc(F["primary_rate_test"]), n(D["excluded_dps"]),
     ...
     f"{D['auprc_relative_gain_full_vs_M2_pct']}%", n(C["metric_cis"]["models"]["M5_full"]["confusion"]["flagged"]), n(C["metric_cis"]["models"]["M5_full"]["confusion"]["tp"]), n(C["metric_cis"]["models"]["M2_structured_history"]["confusion"]["tp"]), n(C["metric_cis"]["models"]["M0_signal_risk"]["confusion"]["tp"]), f(M[C["full_model"]]["sens_flag20"]), f(M["M2_structured_history"]["sens_flag20"]), f(M["M0_signal_risk"]["sens_flag20"]),
...
if SS: must(MS+TABS+APP, "manuscript+appendix(simple score)", f(SS["points_score"]["auroc"]), f(SS["points_score"]["auprc"]), f(SS["logistic"]["auroc"]), f(SS["logistic"]["auprc"]), f"{SS['points_score']['at_20pct_flagged']['per_100_flags']:.1f}", f"{100*SS['share_of_full_model_gain_over_M0_captured']['points_score']:.1f}%")
if CAP: must(MS+TABS+APP, "manuscript+appendix(capacity)", n(CAP["team_weeks"]), str(CAP["mean_contacts_per_team_week"]), str(CAP["mean_disengagements_per_team_week"]), f"{CAP['capacity']['10']['full_model']:.2f}", f"{CAP['capacity']['10']['deployed_risk_score']:.2f}", f"{CAP['capacity']['10']['first_30_days']:.2f}", f"{CAP['capacity']['40']['full_model']:.2f}", f"{CAP['capacity']['40']['deployed_risk_score']:.2f}")
L2 = C.get("levers2"); CS = C.get("clinical_stakes")
if L2: v = L2["L5_second_contact_within_7_days"]["msm_iptw"]; must(MS+TABS+APP, "manuscript+appendix(levers2)", f"{v['odds_ratio']:.2f}", f"{v['ci_95'][0]:.2f} to {v['ci_95'][1]:.2f}", n(L2["L5_second_contact_within_7_days"]["n"]), n(L2["L5_second_contact_within_7_days"]["equipoise_dps"]), f"{L2['L6_same_person_continuity']['within_member_fe']['odds_ratio']:.2f}")
if CS: must(MS+TABS+APP, "manuscript+appendix(clinical stakes)", f"{CS['rates']['y1']['acute']:.1f}", f"{CS['rates']['y0']['acute']:.2f}", f"{CS['irr_acute90']['adjusted']:.2f}", f"{CS['irr_acute90']['adjusted_ci_95'][0]:.2f} to {CS['irr_acute90']['adjusted_ci_95'][1]:.2f}", f"{CS['rates']['y1']['any_acute_pct']:.1f}%", f"{CS['rates']['y0']['any_acute_pct']:.1f}%")

must(MS+TABS+APP, "manuscript+appendix(sequence)", *[f(v["auroc"]) for v in SEQ["models"].values()], *[f(v["auprc"]) for v in SEQ["models"].values()], n(min(v["params"] for v in SEQ["models"].values())), n(max(v["params"] for v in SEQ["models"].values())))

PH = D.get("physician")
if PH:
    k = PH["kappa_q1"]; kh = PH["kappa_q4"]; ws = PH["within_stratum_all"]; mw = PH["model_within_stratum"]; p2 = PH["phase2_all"]
    must(MS+TABS+APP, "manuscript+appendix(physician)", n(PH["forms"]), f"{min(k.values()):.2f}", f"{max(k.values()):.2f}", f"{min(kh.values()):.2f}", f"{max(kh.values()):.2f}", f"{PH['fleiss_q1_core']:.2f}", f"{PH['icc_prognosis_all']:.2f}", f(PH["auroc_model_cases"]), f(PH["auroc_physician_all"]), pc(PH["any_open_need_y1_all"]), pc(PH["any_open_need_y0_all"]), pc(PH["harm_high_y1_all"]), pc(PH["harm_high_y0_all"]), str(PH["phase2_n_all"]), pc(p2["lost_contact"]), pc(p2["needs_met"]), pc(p2["administrative_or_coverage"]), f"{ws['bottom_3_deciles']:.2f}", f"{ws['top_decile']:.2f}", f"{mw['bottom_3_deciles']:.2f}", f"{mw['top_decile']:.2f}")
...
if SE:
    ...
    must(MS+TABS+APP, "manuscript+appendix(staff effects)", n(H_["n_patients"]), str(H_["n_staff"]), n(A_["n_patients"]), str(A_["n_staff"]), pc(H_["mean_outcome_bottom_quartile_staff"]), pc(H_["mean_outcome_top_quartile_staff"]), f"{100*H_['adjusted_gap_top_minus_bottom_quartile']:.1f}", f"{H_['validation']['forecast_coef']:.2f}", f"{A_['validation']['forecast_coef']:.2f}", f"{100*A_['icc']:.1f}%")
if SIV: must(MS+TABS+APP, "manuscript+appendix(staff iv)", n(SIV["n_patients"]), str(SIV["n_staff"]), *[f"{r['late_rd']:+.3f}" for r in SIV["actions"].values()])
AT = C.get("attempts")
if AT: must(MS+TABS+APP, "manuscript+appendix(attempts)", f"{AT['disengagements_with_any_attempt_in_90d_pct']}%", f"{AT['disengagements_with_3plus_attempts_pct']}%", f"{AT['explicit_exit_with_any_attempt_pct']}%", f"{AT['silent_loss_with_any_attempt_pct']}%", f"{D['attempts']['dis_no_attempt_pct']}%")
