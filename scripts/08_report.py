...
axs[0].plot([0,1],[0,1], ls=":", c="k", lw=0.8); axs[0].set(xlabel="1 - specificity", ylabel="Sensitivity", title="a  Receiver operating characteristic"); axs[0].legend(fontsize=6.5, frameon=False, loc="lower right")
axs[1].axhline(y.mean(), ls=":", c="k", lw=0.8); axs[1].set(xlabel="Sensitivity (recall)", ylabel="Positive predictive value", title="b  Precision-recall"); axs[1].legend(fontsize=6.5, frameon=False, loc="upper right")
...
axs[2].plot([0,1],[0,1], ls=":", c="k", lw=0.8); axs[2].set(xlabel="Predicted probability (decile mean)", ylabel="Observed disengagement", title="c  Calibration, test era", xlim=(0,1), ylim=(0,1)); axs[2].legend(fontsize=6.5, frameon=False, loc="upper left")
...
with open(TAB/"table5_sequence_models.md", "w") as f:
    if seq:
        f.write(f"**Table 5. Supplementary architecture comparison on the test era ({seq['n_test']:,} decision points).** Sequence models read the last {seq['sequence_length']} contacts (token dimension {seq['token_dim']}: discipline, modality, timing, and 32 principal components of the note embedding) plus the same nine structured covariates; three seeds, early stopping on a patient-grouped validation fold; predictions averaged over seeds.\n\n| Model | Parameters | AUROC | AUPRC | AUPRC by seed |\n|---|---|---|---|---|\n")
        for k, v in seq["models"].items(): f.write(f"| {k.replace('_',' ')} | {v['params']:,} | {v['auroc']:.3f} | {v['auprc']:.3f} | {', '.join(f'{s:.3f}' for s in v['seed_auprc'])} |\n")
...
if sens:
    ...
    with open(TAB/"tableS3_sensitivity.md", "w") as f:
        f.write("**Supplementary Table S3. Sensitivity of the structured plus contact history model (M2) and the model with note text (M3, TF-IDF stack) to eligibility, split, and outcome definition.** Each row refits both models and evaluates on the test era under the stated specification; embeddings, lexicon, and tags are not included in M3.\n\n| Specification | Training decision points | Test decision points | Test event rate | AUROC, M2 | AUPRC, M2 | AUROC, M3 | AUPRC, M3 | AUPRC gain from text |\n|---|---|---|---|---|---|---|---|---|\n")
        for k, v in sens.items(): f.write(f"| {NM.get(k,k)} | {v['n_train']:,} | {v['n_test']:,} | {100*v['test_event_rate']:.1f}% | {v['M2_structured_history']['auroc']:.3f} | {v['M2_structured_history']['auprc']:.3f} | {v['M3_plus_tfidf_text']['auroc']:.3f} | {v['M3_plus_tfidf_text']['auprc']:.3f} | {v['auprc_gain_text']:+.3f} |\n")
...
if LM and LM.get("headline_landmark"):
    ...
    ks = sorted(LM["landmarks"]); fig, axs = plt.subplots(1, 3, figsize=(12, 3.6))
    ...
    axs[0].axvline(ks.index(LM["headline_landmark"]), c="#aaa", ls=":", lw=1); axs[0].set_xticks(range(len(ks))); axs[0].set_xticklabels([k[:7] for k in ks], rotation=60, fontsize=7); axs[0].set(ylabel="Performance on all contacts after the landmark", title="a  Frozen model by landmark", ylim=(0.5, 1.0)); axs[0].legend(fontsize=7, frameon=False)
    ax2 = axs[0].twinx(); ax2.plot(range(len(ks)), [LM["landmarks"][k]["overall"]["calibration_intercept"] for k in ks], "^--", color="#888", lw=1, label="Calibration intercept"); ax2.set_ylabel("Calibration intercept", color="#888"); ax2.axhline(0, c="#ccc", lw=0.6); ax2.legend(fontsize=7, frameon=False, loc="lower right")
    ...
    axs[1].plot([int(k) for k in ms], [bm[k]["per_100_flags"]/100 for k in ms], "d-", color=COL["M2_structured_history"], label="Disengagements per flag"); axs[1].set(xlabel=f"Months after landmark {LM['headline_landmark']}", title="b  Decay after the headline landmark", ylim=(0, 1)); axs[1].legend(fontsize=7, frameon=False)
    ...
    with open(TAB/"tableS5_landmark.md", "w") as f:
        f.write("**Supplementary Table S5. Frozen-model rolling-landmark validation.** At each month-end landmark the full model without embeddings is trained on eligible decision points dated at least 90 days before the landmark and scores every eligible decision point after it; the flagging threshold is the 80th percentile of training predictions.\n\n| Landmark | Training decision points (events) | Scored decision points (events) | AUROC | AUPRC | Calibration slope | Calibration intercept | Flag rate | Disengagements per 100 flags | Sensitivity at threshold | AUROC, members not in training |\n|---|---|---|---|---|---|---|---|---|---|---|\n")
        for k in ks:
            v = LM["landmarks"][k]; o = v["overall"]; f.write(f"| {k}{' (headline)' if k == LM['headline_landmark'] else ''} | {v['train']['n']:,} ({v['train']['events']:,}) | {o['n']:,} ({o['events']:,}) | {o['auroc']:.3f} | {o['auprc']:.3f} | {o['calibration_slope']:.2f} | {o['calibration_intercept']:+.2f} | {100*o['flag_rate']:.1f}% | {o['per_100_flags']:.1f} | {o['sensitivity_at_threshold']:.3f} | {v['members_not_seen_in_training'].get('auroc', float('nan')):.3f} |\n")
...
if SS:
    ...
    with open(TAB/"tableS6_simple_score.md", "w") as f:
        ...
        f.write(f"\n| Model | AUROC | AUPRC | Disengagements per 100 flags (20%) | Sensitivity (20%) | Share of full-model AUPRC gain over M0 captured |\n|---|---|---|---|---|---|\n| Logistic (8 predictors) | {SS['logistic']['auroc']:.3f} | {SS['logistic']['auprc']:.3f} | {SS['logistic']['at_20pct_flagged']['per_100_flags']:.1f} | {SS['logistic']['at_20pct_flagged']['sensitivity']:.3f} | {100*SS['share_of_full_model_gain_over_M0_captured']['logistic']:.1f}% |\n| Integer points score | {SS['points_score']['auroc']:.3f} | {SS['points_score']['auprc']:.3f} | {SS['points_score']['at_20pct_flagged']['per_100_flags']:.1f} | {SS['points_score']['at_20pct_flagged']['sensitivity']:.3f} | {100*SS['share_of_full_model_gain_over_M0_captured']['points_score']:.1f}% |\n")
if CAP:
    with open(TAB/"tableS7_capacity.md", "w") as f:
        f.write(f"**Supplementary Table S7. True disengagements reached per care team per week at fixed re-engagement capacity (test era; {CAP['team_weeks']:,} team-weeks; mean {CAP['mean_contacts_per_team_week']} contacts and {CAP['mean_disengagements_per_team_week']} disengagements per team-week).**\n\n| Weekly attempts per team | Full model | Structured + history | Deployed risk score | All first-30-day contacts | Random |\n|---|---|---|---|---|---|\n")
        for cap, v in CAP["capacity"].items(): f.write(f"| {cap} | {v['full_model']:.2f} | {v['structured_history']:.2f} | {v['deployed_risk_score']:.2f} | {v['first_30_days']:.2f} | {v['random']:.2f} |\n")
    canon["derived"]["capacity_10"] = CAP["capacity"]["10"]
if FAIR:
    with open(TAB/"tableS8_fairness.md", "w") as f:
        f.write("**Supplementary Table S8. Full-model performance by subgroup at the single overall 20% threshold (test era).** Differences in sensitivity and positive predictive value versus the reference group carry 95% member-bootstrap intervals.\n\n| Dimension | Group | n | Event rate | AUROC | AUPRC | Calibration slope | Calibration intercept | Flag rate | Sensitivity | PPV | Sensitivity difference vs reference (95% CI) | PPV difference vs reference (95% CI) |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|\n")
        for dim, blk in FAIR["groups"].items():
            for lev, v in blk["levels"].items():
                ...
                f.write(f"| {dim.replace('_',' ')} | {lev}{' (reference)' if lev == blk['reference'] else ''} | {v['n']:,} | {100*v['event_rate']:.1f}% | {m['auroc']:.3f} | {m['auprc']:.3f} | {m['calibration_slope']:.2f} | {m['calibration_intercept']:+.2f} | {100*m['flag_rate']:.1f}% | {m['sensitivity']:.3f} | {m['ppv']:.3f} | {f'{sd[0]:+.3f} to {sd[1]:+.3f}' if sd else 'reference'} | {f'{pd_[0]:+.3f} to {pd_[1]:+.3f}' if pd_ else 'reference'} |\n")
...
if AB:
    with open(TAB/"tableS10_ablation.md", "w") as f:
        f.write(f"**Supplementary Table S10. Feature-group ablation and learner variants of the full model (test era, {models['n_test']:,} decision points).** Leave-one-group-out removes one predictor block from the full model; only-one-group fits the block alone. Logistic regression uses the same standardized features with L2 penalty (C = 0.1).\n\n| Specification | AUROC | AUPRC | Change in AUPRC from full model |\n|---|---|---|---|\n| Full model (gradient boosting) | {AB['full_model']['auroc']:.3f} | {AB['full_model']['auprc']:.3f} | |\n")
        ...
        for g, v in AB["leave_one_group_out"].items(): f.write(f"| Without {NM[g]} | {v['auroc']:.3f} | {v['auprc']:.3f} | {-v['auprc_drop']:+.3f} |\n")
        for g, v in AB["only_one_group"].items(): f.write(f"| Only {NM[g]} | {v['auroc']:.3f} | {v['auprc']:.3f} | {v['auprc'] - AB['full_model']['auprc']:+.3f} |\n")
        v = AB["logistic_regression_same_features"]; f.write(f"| Logistic regression, same features | {v['auroc']:.3f} | {v['auprc']:.3f} | {v['auprc'] - AB['full_model']['auprc']:+.3f} |\n")
        for g, v in AB["hyperparameter_variants"].items(): f.write(f"| Gradient boosting, {g.replace('_',' ')} | {v['auroc']:.3f} | {v['auprc']:.3f} | {v['auprc'] - AB['full_model']['auprc']:+.3f} |\n")
...
if RF:
    with open(TAB/"tableS11_quarterly_refit.md", "w") as f:
        ...
        for q, v in RF["refit_by_quarter"].items():
            w = RF["frozen_by_quarter"].get(q, {}); f.write(f"| {q} | {v['n']:,} | {v['auroc']:.3f} | {v['calibration_intercept']:+.2f} | {v['per_100_flags']:.1f} | {w.get('auroc', float('nan')):.3f} | {w.get('calibration_intercept', float('nan')):+.2f} | {w.get('per_100_flags', float('nan')):.1f} |\n")
