...
with open(TAB/"tableS21_levers.md", "w") as f:
    ...
    for k, v in lev["exposures"].items():
        ...
        else: skipped.append(f"{nm[0].lower() + nm[1:]} ({100*v['prevalence']:.1f}% of decision points; {v['equipoise_dps']:,} in equipoise)")
    if skipped: f.write(f"\nNot shown: {'; '.join(skipped)}. These exposures had too few decision points in the equipoise region for either estimator, because the members who received them did not overlap in measured characteristics with members who did not.\n")
...
if MC:
    ...
    with open(TAB/"tableS22_contrasts.md", "w") as f:
        f.write(f"**Supplementary Table S22. Pre-specified contrasts between models on the test era ({models['n_test']:,} decision points).** Each entry is the difference in the metric between the two models, with a 95% percentile interval from 1,000 bootstrap resamples of members; operating-point metrics use each model's own 80th-percentile threshold (20% of contacts flagged).\n\n| Contrast | AUPRC | AUROC | Net benefit, 20% flagged | PPV, 20% flagged |\n|---|---|---|---|---|\n")
        for k, lab in [("full_vs_structured_history", f"{LABEL[FULL]} vs {LABEL['M2_structured_history']} (primary)"), ("text_vs_structured_history", f"{LABEL['M3_plus_tfidf_text']} vs {LABEL['M2_structured_history']}"), ("structured_history_vs_signal", f"{LABEL['M2_structured_history']} vs {LABEL['M0_signal_risk']} (secondary)"), ("full_vs_structured", f"{LABEL[FULL]} vs {LABEL['M1_structured']}")]:
            f.write(f"| {lab} | {ci(C[k],'auprc')} | {ci(C[k],'auroc')} | {ci(C[k],'nb_flag20')} | {ci(C[k],'ppv_flag20')} |\n")
    TC = J("table_cis.json")
    if TC:
        ...
        with open(TAB/"tableS23_secondary_outcomes.md", "w") as f:
            f.write(f"**Supplementary Table S23. Secondary outcomes on the test era ({models['n_test']:,} decision points; {models['n_test_patients']:,} members).** Models were refit for each outcome with the frozen hyperparameters. Operating-point metrics use each model's own 80th-percentile threshold (20% of contacts flagged). Parentheses are 95% percentile intervals from 500 bootstrap resamples of members.\n\n| Outcome | Event rate | Model | AUROC (95% CI) | AUPRC (95% CI) | Sensitivity (95% CI) | PPV (95% CI) | Net benefit (95% CI) |\n|---|---|---|---|---|---|---|---|\n")
...
if PR:
    ...
    with open(TAB/"tableS15_physician_review.md", "w") as f:
        ...
        cif = lambda c: f" ({c[0]:.3f} to {c[1]:.3f})" if c else ""
    with open(TAB/"tableS16_review_prognosis.md", "w") as f:
        f.write(f"**Supplementary Table S16. Physician prognosis against realized disengagement on the same cases (blinded physician review; design and reviewer roles as in Supplementary Table S15).** {PR['design_note']} Parentheses are 95% percentile intervals from 1,000 bootstrap resamples of cases.\n\n| Raters | AUROC of mean physician prognosis (95% CI) | AUROC of the full model on the same cases (95% CI) | Difference (model minus physician), 95% bootstrap CI |\n|---|---|---|---|\n| All three reviewers | {pv['auroc_physician']:.3f}{cif(pv.get('auroc_physician_ci_95'))} | {pv['auroc_model']:.3f}{cif(pv.get('auroc_model_ci_95'))} | {pv['difference_model_minus_physician']:+.3f} ({pv['ci_95'][0]:.3f} to {pv['ci_95'][1]:.3f}) |\n")
        if wo: f.write(f"| Reviewers B and C only | {wo['auroc_physician']:.3f}{cif(wo.get('auroc_physician_ci_95'))} | {wo['auroc_model']:.3f}{cif(wo.get('auroc_model_ci_95'))} | |\n")
        ...
        for st, v in PR.get("prognosis_auroc_within_stratum_all", {}).items(): f.write(f"| All reviewers, within model stratum {st.replace('_',' ')} | {v:.3f}{cif(PR.get('prognosis_auroc_within_stratum_all_ci', {}).get(st))} | {PR.get('model_auroc_within_stratum', {}).get(st, float('nan')):.3f}{cif(PR.get('model_auroc_within_stratum_ci', {}).get(st))} | |\n")
    ...
    with open(TAB/"tableS17_review_open_need.md", "w") as f:
        f.write("**Supplementary Table S17. Open clinical need and harm if lost at the index contact, share of cases (mean over reviewers per case), by realized outcome (blinded physician review; design and reviewer roles as in Supplementary Table S15).**\n\n| Item | Disengaged | Retained |\n|---|---|---|\n")
    ...
    with open(TAB/"tableS18_review_status.md", "w") as f:
        f.write("**Supplementary Table S18. Engagement status at the index contact by realized outcome, share of phase 1 forms (blinded physician review; design and reviewer roles as in Supplementary Table S15).**\n\n| Status | Disengaged | Retained |\n|---|---|---|\n")
    ...
    with open(TAB/"tableS19_review_reasons.md", "w") as f:
        f.write(f"**Supplementary Table S19. Adjudicated reason for disengagement (blinded physician review, phase 2; one reviewer per case; {PR.get('phase2_double_coded_agreement', {}).get('n_double_coded', 0)} cases double-coded with {100*(PR.get('phase2_double_coded_agreement', {}).get('percent_agreement') or 0):.0f}% agreement).**\n\n| Reason | Share of {PR['phase2_n']} cases |\n|---|---|\n")
    ...
    with open(TAB/"tableS20_review_patterns.md", "w") as f:
        f.write("**Supplementary Table S20. Clinical meaningfulness of the 20 note-text patterns (1 to 5; mean of three reviewers; blinded physician review, phase 3).**\n\n| Pattern | Mean | SD |\n|---|---|---|\n")
