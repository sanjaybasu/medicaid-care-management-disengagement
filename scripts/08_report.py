...
def t1_block(d):
    ...
    rows = {"Decision points (completed contacts), n": f"{len(d):,}", "Patients, n": f"{d.person_id.nunique():,}", "Contacts per patient, median (IQR)": f"{d.groupby('person_id').size().median():.0f} ({d.groupby('person_id').size().quantile(.25):.0f}-{d.groupby('person_id').size().quantile(.75):.0f})",
            "Age at first contact, mean (SD), years": f"{first.age.mean():.1f} ({first.age.std():.1f})", "Female, % of patients": pct(first.gender == "Female"),
            "Black or African American, % of patients": pct(first.race == "Black or African American"), "White, % of patients": pct(first.race == "White"), "Hispanic, % of patients": pct(first.race == "Hispanic"), "Asian, % of patients": pct(first.race == "Asian"), "Other or unknown race, % of patients": pct(~first.race.isin(["Black or African American","White","Hispanic","Asian"])),
            "Virginia, % of patients": pct(first.state == "VIRGINIA"), "Washington, % of patients": pct(first.state == "WASHINGTON"), "Ohio, % of patients": pct(first.state == "OHIO"),
            ...
            "Any of diabetes, hypertension, heart failure, COPD, or asthma, % of patients": pct((first.diabetes == 1) | (first.htn == 1) | (first.chf == 1) | (first.copd == 1) | (first.asthma == 1)), "Any behavioral health condition, % of patients": pct(first.any_bh == 1), "Substance use disorder, % of patients": pct(first.sud == 1), "Hypertension, % of patients": pct(first.htn == 1), "Diabetes, % of patients": pct(first.diabetes == 1), "Asthma or COPD, % of patients": pct((first.asthma == 1) | (first.copd == 1)), "Polypharmacy, % of patients": pct(first.polypharmacy == 1), "No primary care visit in prior 10 months, % of patients": pct(first.no_pcp_last_10mo == 1),
            ...
            "Disengagement within 90 days, % of contacts": pct(d.y_primary == 1), "Explicit exit status within 90 days, % of contacts": pct(d.y_explicit == 1), "Silent loss within 90 days, % of contacts": pct(d.y_silent == 1), "No contact within 30 days, % of contacts": pct(d.y_30 == 1), "Program completion within 90 days, % of contacts": pct(d.grad_90 == 1)}
    return rows
...
canon = {"generated_from": ["flow.json","models_primary.json","subgroups.json","levers.json","hyperparameters.json","features_meta.json","sequence_models.json","sensitivity.json"], "full_model": FULL, "flow": flow, "models": models, "subgroups": subs, "levers": lev, "hyperparameters": hp, "features_meta": fmeta, "sequence_models": seq, "sensitivity": sens,
         ...
         "excluded_dps": int(len(F) - flow["eligible_90_enrolled_days"]), "excluded_pct": round(100*(len(F) - flow["eligible_90_enrolled_days"])/len(F), 1)}}
...
with open(TAB/"tableS12_sequence_models.md", "w") as f:
    if seq:
        f.write(f"**Supplementary Table S12. Architecture comparison on the test era ({seq['n_test']:,} decision points).** Sequence models read the last {seq['sequence_length']} contacts (token dimension {seq['token_dim']}: discipline, modality, timing, and 32 principal components of the note embedding) plus the same nine structured covariates; three seeds, early stopping on a patient-grouped validation fold; predictions averaged over seeds. Parentheses are 95% patient-bootstrap intervals (1,000 resamples).\n\n| Model | Parameters | AUROC (95% CI) | AUPRC (95% CI) | AUPRC by seed |\n|---|---|---|---|---|\n")
...
if LM and LM.get("headline_landmark"):
    ...
    with open(TAB/"tableS5_landmark.md", "w") as f:
        f.write("**Supplementary Table S5. Frozen-model rolling-landmark validation.** At each month-end landmark the full model without embeddings is trained on eligible decision points dated at least 90 days before the landmark and scores every eligible decision point after it; the flagging threshold is the 80th percentile of training predictions. Parentheses are 95% patient-bootstrap intervals (300 resamples).\n\n| Landmark | Training decision points (events) | Scored decision points (events) | AUROC | AUPRC | Calibration slope | Calibration intercept | Flag rate | Disengagements per 100 flags | Sensitivity at threshold | AUROC, patients not in training |\n|---|---|---|---|---|---|---|---|---|---|---|\n")
        for k in ks:
...
if FAIR:
    with open(TAB/"tableS8_fairness.md", "w") as f:
        f.write("**Supplementary Table S8. Full-model performance by subgroup at the single overall 20% threshold (test era).** Parentheses are 95% patient-bootstrap intervals within each group (300 resamples); differences versus the reference group carry 95% patient-bootstrap intervals.\n\n| Dimension | Group | n | Event rate | AUROC | AUPRC | Calibration slope | Calibration intercept | Flag rate | Sensitivity | PPV | Sensitivity difference vs reference (95% CI) | PPV difference vs reference (95% CI) |\n|---|---|---|---|---|---|---|---|---|---|---|---|---|\n")
...
if LV2 or CS:
    with open(TAB/"tableS9_levers2_clinical_stakes.md", "w") as f:
        if LV2:
            f.write(f"**Supplementary Table S9a. Care-team actions as candidate levers (outcome: no completed contact in days 31 to 120 and no program completion by day 120; {canon['levers']['n_lever_eligible']:,} eligible decision points for the in-person exposure, base rate {100*canon['levers']['base_rate']:.1f}%).** Rows for the second-contact and continuity exposures measure the outcome from the next contact. Pharmacist review, therapy session, and change of discipline had no decision points in the equipoise region and are not shown.\n\n| Exposure | Index decision points | Prevalence | Equipoise decision points | Within-patient OR (95% CI) | MSM OR (95% CI) | MSM risk difference | Detectable OR at 80% power (MSM) | Gate 1 | Gate 2 | Risk-difference bounds, gamma 1.25 |\n|---|---|---|---|---|---|---|---|---|---|---|\n")
            LEVD = canon["levers"]; L1 = LEVD["exposures"].get("L1_inperson", {})
            if "within_patient_fe" in L1:
                fe, ms, b = L1["within_patient_fe"], L1["msm_iptw"], L1["sensitivity_bounds_risk_difference"]["gamma_1.25"]
                f.write(f"| In-person versus remote contact (outcome measured from the index contact) | {LEVD['n_lever_eligible']:,} | {100*L1['prevalence']:.1f}% | {L1['equipoise_dps']:,} | {fe['odds_ratio']:.2f} ({fe['ci_95'][0]:.2f} to {fe['ci_95'][1]:.2f}) | {ms['odds_ratio']:.2f} ({ms['ci_95'][0]:.2f} to {ms['ci_95'][1]:.2f}) | {ms['risk_difference']:+.3f} | {fe.get('mde_or_80pct_power', '')} | {'Pass' if L1['gate1_pretrend']['passed'] else 'Fail'} | {'Agree' if L1['gate2_agreement'] else 'Disagree'} | {b[0]:+.3f} to {b[1]:+.3f} |\n")
            for k, nm in [("L5_second_contact_within_7_days","Second contact within 7 days of the first contact"),("L6_same_person_continuity","Next contact by the same care team patient")]:
        ...
        if CS:
            f.write(f"\n**Supplementary Table S9b. Acute care events in days 1 to 90 after test-era decision points ({CS['n_test_dps']:,} decision points; {CS['members']:,} patients), disengaged versus retained.** Events per 100 decision points; incidence rate ratios from Poisson regression with patient-clustered standard errors, adjusted for risk percentile, days since activation, prior acute events, age, behavioral health and substance use flags, state, and discipline.\n\n| Outcome | Disengaged (n = {CS['rates']['y1']['n']:,}) | Retained (n = {CS['rates']['y0']['n']:,}) | Unadjusted IRR (95% CI) | Adjusted IRR (95% CI) |\n|---|---|---|---|---|\n")
...
if MC:
    with open(TAB/"table2_model_performance.md", "w") as f:
        f.write(f"**Table 2. Prediction of disengagement within 90 days on the test era ({models['n_test']:,} decision points; {models['n_test_patients']:,} patients; {models['test_event_rate']*100:.1f}% event rate).** Operating-point metrics use each model's own 80th-percentile threshold (20% of contacts flagged). Intervals are 95% percentile intervals from 1,000 bootstrap resamples of patients. Disengagements per 100 flags equals 100 times the positive predictive value (PPV); NPV, negative predictive value. For comparison of interval width, a bootstrap of decision points rather than patients gave, for the full model, AUROC {MC['M5_full_decision_point_bootstrap']['auroc'][0]:.3f} to {MC['M5_full_decision_point_bootstrap']['auroc'][1]:.3f} and AUPRC {MC['M5_full_decision_point_bootstrap']['auprc'][0]:.3f} to {MC['M5_full_decision_point_bootstrap']['auprc'][1]:.3f}.\n\n| Model | AUROC (95% CI) | AUPRC (95% CI) | Brier (95% CI) | Calibration slope (95% CI) | Sensitivity (95% CI) | Specificity (95% CI) | PPV (95% CI) | NPV (95% CI) | F1 (95% CI) | Net benefit (95% CI) |\n|---|---|---|---|---|---|---|---|---|---|---|\n")
        ...
        for m in ["M0_signal_risk","M1_structured","M2_structured_history","M3_plus_tfidf_text","M3b_plus_lexicon_tags","M4_plus_embeddings","M5_full","text_only_tfidf","embeddings_only","seq_GRU","seq_Transformer","seq_Mamba_SSM"]:
            if m in MC["models"]:
                ...
                f.write(f"| {lab} | {c(v,'auroc')} | {c(v,'auprc')} | {c(v,'brier')} | {c(v,'calibration_slope',2)} | {c(v,'sensitivity')} | {c(v,'specificity')} | {c(v,'ppv')} | {c(v,'npv')} | {c(v,'f1')} | {c(v,'net_benefit',4)} |\n")
    with open(TAB/"tableS17_contrasts.md", "w") as f:
        f.write(f"**Supplementary Table S17. Pre-specified contrasts between models on the test era ({models['n_test']:,} decision points).** Each entry is the difference in the metric between the two models, with a 95% percentile interval from 1,000 bootstrap resamples of patients; operating-point metrics use each model's own 80th-percentile threshold (20% of contacts flagged).\n\n| Contrast | AUPRC | AUROC | Net benefit, 20% flagged | PPV, 20% flagged |\n|---|---|---|---|---|\n")
        for k, lab in [("full_vs_structured_history", f"{LABEL[FULL]} vs {LABEL['M2_structured_history']} (primary)"), ("text_vs_structured_history", f"{LABEL['M3_plus_tfidf_text']} vs {LABEL['M2_structured_history']}"), ("structured_history_vs_signal", f"{LABEL['M2_structured_history']} vs {LABEL['M0_signal_risk']} (secondary)"), ("full_vs_structured", f"{LABEL[FULL]} vs {LABEL['M1_structured']}")]:
            f.write(f"| {lab} | {ci(C[k],'auprc')} | {ci(C[k],'auroc')} | {ci(C[k],'nb_flag20')} | {ci(C[k],'ppv_flag20')} |\n")
    with open(TAB/"table4_confusion.md", "w") as f:
        f.write(f"**Table 4. Classification of test-era decision points at the 20% flagging threshold ({models['n_test']:,} decision points; {MC['events']:,} disengagements).** Each model flags the 20% of contacts with the highest predicted probability (its own 80th-percentile threshold). Counts are decision points; rates carry 95% percentile intervals from 1,000 bootstrap resamples of patients.\n\n| Model | Flagged | True positives | False positives | False negatives | True negatives | Sensitivity (95% CI) | Specificity (95% CI) | PPV (95% CI) | NPV (95% CI) |\n|---|---|---|---|---|---|---|---|---|---|\n")
        for m in ["M0_signal_risk", "M2_structured_history", "M5_full"]:
            v = MC["models"][m]; k = v["confusion"]
            f.write(f"| {LABEL[m]} | {k['flagged']:,} ({100*k['flagged']/k['n']:.1f}%) | {k['tp']:,} | {k['fp']:,} | {k['fn']:,} | {k['tn']:,} | {c(v,'sensitivity')} | {c(v,'specificity')} | {c(v,'ppv')} | {c(v,'npv')} |\n")
    with open(TAB/"tableS19_youden.md", "w") as f:
        f.write(f"**Supplementary Table S19. Operating points on the test era: the capacity-based 20% flagging threshold used throughout against the Youden-index threshold (the probability cutpoint that maximizes sensitivity plus specificity minus one).**\n\n| Model | Threshold rule | Probability cutpoint | Flag rate | Sensitivity | Specificity | PPV |\n|---|---|---|---|---|---|---|\n")
        for m in ["M0_signal_risk", "M2_structured_history", "M5_full"]:
    ...
    cf0, cf2, cf5 = (MC["models"][m]["confusion"] for m in ["M0_signal_risk","M2_structured_history","M5_full"]); canon["derived"]["events_captured_ratio_M2_vs_M0_at_20pct"] = round(cf2["tp"]/cf0["tp"], 1); canon["derived"]["events_captured_gain_full_vs_M2_at_20pct_confusion"] = cf5["tp"] - cf2["tp"]; canon["derived"]["per_100_flags_gain_full_vs_M2"] = round(100*(MC["models"]["M5_full"]["ppv"]["estimate"] - MC["models"]["M2_structured_history"]["ppv"]["estimate"]), 1); canon["derived"]["lexicon_tags_share_of_text_auprc_gain_pct"] = round(100*(MC["models"]["M3b_plus_lexicon_tags"]["auprc"]["estimate"] - MC["models"]["M2_structured_history"]["auprc"]["estimate"])/C["full_vs_structured_history"]["auprc"]["diff"])
    canon["derived"]["youden"] = {m: MC["models"][m]["youden"] for m in ["M0_signal_risk","M2_structured_history","M5_full"]}
    TC = J("table_cis.json")
    if TC:
        canon["table_cis"] = TC; canon["generated_from"].append("table_cis.json")
        def tc(v, k, d=3): return f"{v[k]['estimate']:.{d}f} ({v[k]['ci_95'][0]:.{d}f} to {v[k]['ci_95'][1]:.{d}f})"
        with open(TAB/"tableS18_secondary_outcomes.md", "w") as f:
            f.write(f"**Supplementary Table S18. Secondary outcomes on the test era ({models['n_test']:,} decision points; {models['n_test_patients']:,} patients).** Models were refit for each outcome with the frozen hyperparameters. Operating-point metrics use each model's own 80th-percentile threshold (20% of contacts flagged). Parentheses are 95% percentile intervals from 500 bootstrap resamples of patients.\n\n| Outcome | Event rate | Model | AUROC (95% CI) | AUPRC (95% CI) | Sensitivity (95% CI) | PPV (95% CI) | Net benefit (95% CI) |\n|---|---|---|---|---|---|---|---|\n")
            for yc, b in TC["secondary_outcomes"].items():
                for m in ["M2_structured_history", "M5_full"]:
                    v = b[m]; f.write(f"| {b['label']} | {100*b['test_event_rate']:.1f}% ({b['events']:,} of {b['n']:,}) | {LABEL[m]} | {tc(v,'auroc')} | {tc(v,'auprc')} | {tc(v,'sensitivity_20')} | {tc(v,'ppv_20')} | {tc(v,'net_benefit_20',4)} |\n")
        with open(TAB/"table3_subgroups_loso.md", "w") as f:
            f.write(f"**Table 3. Discrimination by subgroup on the test era and in leave-one-state-out validation.** Test-era rows use the structured plus contact history model (M2) and the full model (M5) on the {models['n_test']:,} test-era decision points. Leave-one-state-out rows train on two states and test on the third over all eligible decision points, comparing M2 with the model that adds note text, lexicon, and tags (M3b), because that model can be refit within compute limits. Parentheses are 95% percentile intervals from 500 bootstrap resamples of patients within the row. CHW, community health worker; CC, care coordinator; CPhT, pharmacy technician; PharmD, clinical pharmacist.\n\n| Subgroup | n | Events | AUROC, M2 (95% CI) | AUROC, with note text (95% CI) | AUROC gain (95% CI) | AUPRC, M2 (95% CI) | AUPRC, with note text (95% CI) |\n|---|---|---|---|---|---|---|---|\n")
            NICE = {"by_state": "State", "by_race": "Race and ethnicity", "by_days_since_activation": "Days since activation", "by_activating_discipline": "Discipline of contact"}
            for grp, rows in TC["subgroups"].items():
                for k, v in rows.items():
                    lab = k.replace("_", " to ") if grp == "by_days_since_activation" else k.title() if grp == "by_state" else k
                    f.write(f"| {NICE[grp]}: {lab} | {v['n']:,} | {v['events']:,} | {tc(v['M2_structured_history'],'auroc')} | {tc(v['M5_full'],'auroc')} | {v['auroc_gain']['estimate']:+.3f} ({v['auroc_gain']['ci_95'][0]:+.3f} to {v['auroc_gain']['ci_95'][1]:+.3f}) | {tc(v['M2_structured_history'],'auprc')} | {tc(v['M5_full'],'auprc')} |\n")
            for k, v in TC["leave_one_state_out"].items():
                f.write(f"| Leave-one-state-out: held-out {k.replace('test_','').title()} | {v['n']:,} | {v['events']:,} | {tc(v['structured_history'],'auroc')} | {tc(v['full_text'],'auroc')} | | {tc(v['structured_history'],'auprc')} | {tc(v['full_text'],'auprc')} |\n")
        canon["derived"]["table3_ci"] = {"loso_full_text_auroc": {k: v["full_text"]["auroc"] for k, v in TC["leave_one_state_out"].items()}, "subgroup_full_auroc": {f"{g}:{k}": v["M5_full"]["auroc"] for g, rows in TC["subgroups"].items() for k, v in rows.items()}}
        if "equalized_odds" in TC:
            with open(TAB/"tableS20_equalized_odds.md", "w") as f:
                f.write(f"**Supplementary Table S20. Equalized odds at the single overall 20% flagging threshold on the test era.** True positive rate (sensitivity) and false positive rate by group at one common threshold; the equalized odds ratio is the smaller of the ratios of the lowest to the highest group true positive rate and false positive rate (1.0 is exact parity). Parentheses are 95% percentile intervals from 500 bootstrap resamples of patients.\n\n| Model | Dimension | Group | n | True positive rate (95% CI) | False positive rate (95% CI) | Equalized odds ratio (95% CI) |\n|---|---|---|---|---|---|---|\n")
                for key, v in TC["equalized_odds"].items():
                    m, dim = key.split(":"); first = True
                    for lev, g in v["groups"].items():
                        eo_s = f"{v['equalized_odds_ratio']:.3f} ({v['equalized_odds_ratio_ci_95'][0]:.3f} to {v['equalized_odds_ratio_ci_95'][1]:.3f})" if first else ""; first = False
                        f.write(f"| {LABEL[m]} | {'Race and ethnicity' if dim == 'race' else 'State'} | {lev.title() if dim == 'state' else lev} | {g['n']:,} | {g['tpr']:.3f} ({g['tpr_ci_95'][0]:.3f} to {g['tpr_ci_95'][1]:.3f}) | {g['fpr']:.3f} ({g['fpr_ci_95'][0]:.3f} to {g['fpr_ci_95'][1]:.3f}) | {eo_s} |\n")
            canon["derived"]["equalized_odds"] = {k: {"ratio": v["equalized_odds_ratio"], "ci": v["equalized_odds_ratio_ci_95"], "tpr": {lev: g["tpr"] for lev, g in v["groups"].items()}, "fpr": {lev: g["fpr"] for lev, g in v["groups"].items()}} for k, v in TC["equalized_odds"].items()}
...
if RF:
    with open(TAB/"tableS11_quarterly_refit.md", "w") as f:
        ...
        f.write(f"**Supplementary Table S11. Quarterly refitting versus a single frozen model, pooled over decision points dated after 31 March 2025 ({RF['pooled_n']:,} decision points).** {RF['design']}. Intervals are 95% patient-bootstrap (500 resamples).\n\n| Deployment | AUROC | AUPRC | Calibration slope | Calibration intercept | Flag rate | Disengagements per 100 flags | Sensitivity |\n|---|---|---|---|---|---|---|---|\n")
...
RS = J("roster_scope.json")
if RS: canon["roster_scope"] = RS; canon["generated_from"].append("roster_scope.json"); canon["derived"]["roster"] = {"pcp_count": RS["total"]["pcp_count"], "active_tins": RS["total"]["active_tins"], "active_provider_entities": RS["total"]["active_provider_entities"], "as_of": RS["as_of"], "by_state": {k: v for k, v in RS["by_state"].items()}}
...
PR = J("physician_review.json"); PM = J("physician_sample_meta.json")
if PM: canon["physician_sample"] = PM; canon["generated_from"].append("physician_sample_meta.json")
if PR and PR.get("design") == "single_phase_v2":
    canon["physician_review"] = PR; canon["generated_from"].append("physician_review.json")
    ITEM = {"q1_open_need_medication": "Medication gap", "q1_open_need_referral_or_appointment": "Pending referral or appointment", "q1_open_need_uncontrolled_condition": "Uncontrolled condition", "q1_open_need_social_crisis": "Active social crisis", "q1_open_need_behavioral_health": "Behavioral health need", "q1_no_open_need": "No open need", "any_open_need": "Any open clinical need", "harm_high": "Harm if lost rated high"}
    sc = lambda v: f"{100*v['share']:.1f}% ({100*v['ci_95'][0]:.1f} to {100*v['ci_95'][1]:.1f})"
    with open(TAB/"tableS15_physician_open_need.md", "w") as f:
        f.write(f"**Supplementary Table S15. Blinded physician review of clinical stakes at the last completed contact: {PR['cases_with_form']} test-era patients, {PM['per_group']} whose last contact was followed by disengagement and {PM['per_group']} whose last contact was followed by program completion.** Reviewers saw the record only up to the index contact and did not know the group. Shares are the mean over readers per case; parentheses are 95% percentile intervals from 1,000 bootstrap resamples of cases.\n\n| Item | Disengaged | Completed the program | Difference, percentage points (95% CI) |\n|---|---|---|---|\n")
        for k, lab in ITEM.items():
            d = PR["differences"][k]; f.write(f"| {lab} | {sc(PR['groups']['disengaged'][k])} | {sc(PR['groups']['completed'][k])} | {100*d['risk_difference_disengaged_minus_completed']:+.1f} ({100*d['ci_95'][0]:+.1f} to {100*d['ci_95'][1]:+.1f}) |\n")
    with open(TAB/"tableS16_physician_agreement.md", "w") as f:
        f.write(f"**Supplementary Table S16. Agreement between physician reviewers on the {PM['double_read_cases']} double-read cases.**\n\n| Item | Double-read cases | Percent agreement | Cohen's kappa |\n|---|---|---|---|\n")
        for k, lab in ITEM.items():
            a = PR["agreement"][k]; f.write(f"| {lab} | {a['n_double_read']} | {'' if a['percent_agreement'] is None else f'{100*a['percent_agreement']:.1f}%'} | {'' if a['kappa'] is None else f'{a['kappa']:.2f}'} |\n")
    canon["derived"]["physician"] = {"any_open_need": {g: PR["groups"][g]["any_open_need"]["share"] for g in PR["groups"]}, "harm_high": {g: PR["groups"][g]["harm_high"]["share"] for g in PR["groups"]}}
    json.dump(canon, open(R/"canonical.json", "w"), indent=1, default=str); print("physician review rendered")
json.dump(canon, open(R/"canonical.json", "w"), indent=1, default=str); print("canonical written")
