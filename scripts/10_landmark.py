...
df = F.merge(O[["decision_id","eligible_landmark","y_primary"]], on="decision_id"); df = df[df.eligible_landmark == 1].reset_index(drop=True); df["enc_date"] = pd.to_datetime(df.enc_date)
